...
void print_node(FILE *stream, const struct node* node) {
    fprintf(stream, "node id: %d", node->id);
}
...
int test_add(struct database* db, 
    ...
    fprintf(test_result, "k: %X\n", &k);
    for (size_t i = 0; i < k; i++) {
        fprintf(test_result, "node: %d\n", nodes_num[i]);
        fprintf(test_result, "file: %d\n", file_size[i]);
    }
    fprintf(test_result, "time: %d\n", time);
